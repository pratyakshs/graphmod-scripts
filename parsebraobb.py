...
for linenum, line in enumerate(lines):
	if 'Reading problem' in line:
		problem_name = line.split()[2]
		try:
			c_star = -SOL.get(problem_name[:-4])
		except:
			c_star = float('nan')
...
if marker1 == -1:
	...
	for i in range(marker1, marker2 + 1):
		...
		cost[i - marker1] = float('{0:.4f}'.format(float(row[j + 4])*logE))
	j = 0
	cols = [0] * 6
	labels = ['Cost', 'Nodes', 'C/C*', 'Time']
	for i, t in enumerate([10.0, 60.0, 600.0, 3600.0, 10800.0, 21600.0]):
		...
		if j < rows:
			if time[j] < t:	
				cols[i] = [cost[j], ornodes[j] + andnodes[j], "{0:.4f}".format(cost[j]/c_star), time[j]]
			else:
				cols[i] = [float('nan'), float('nan'), float('nan'), t]

	for i, c in enumerate(cols):
		if c == 0:
			cols[i] = [cost[rows-1], ornodes[rows-1]+andnodes[rows-1], "{0:.4f}".format(cost[rows-1]/c_star), time[rows-1]]
	for j in range(0, 4):
		print('&', labels[j], end = ' ')
		for k in range(0, 6):
			print('&', cols[k][j], end = ' ')
